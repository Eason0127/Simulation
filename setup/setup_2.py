...
def IPR(Measured_amplitude, distance, k_max, pixelSize, W, H, numPixels):
    update_phase = [] # 存储更新的相位
    last_field = None # 存储最后输出的场
    # 这一步是为了更新support区域做准备，得到的blank区域将会取代样本外的区域
    plane_wave = np.ones_like(measured_amplitude, dtype=complex)
    blank_field = angular_spectrum_method(plane_wave, pixelSize, -distance, W, H, numPixels)
    S = np.ones_like(Measured_amplitude, dtype=bool)  # 初始全图约束
    for k in range(k_max):
        ...
        if np.any(mask_bg):
            ...
            m = 1.0  # 或者跳过替换
        field2_updated = field2.copy() # 确保替换操作不会修改原始的 field2
        field2_updated[~S] = blank_field[~S] * m # 背景区域的设定
        field2 = field2_updated # 更新样本场
        ...
        field22 = field2.copy()
        field22[S] = amp_field2[S] * np.exp(1j * phase_field2[S])
...
pitch_size = 5.86e-6
num_pixel = 1216
z_list = np.linspace(8e-2, 1.2e-1, 50)
