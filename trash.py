...
noise_electrons = [50,100,150,200,250,300,350,400,450,500,550,600,650,700,750] # Choose the number of noise electrons
for i in noise_electrons:
    noise_standard = i / scaling_factor  # Transform noise form scale of electrons to scale of intensity
    ...
    rec_field, rms_errors, ssim_errors = IPR(am_hologram_with_noise, z2, 400, 1.5e-20, sensor_pixel_sizes[1], W_sen,
                                             H_sen, numPixels_sensor, am_object_field_down)
    am_rec_field = np.abs(rec_field)
    # plot_image(am_rec_field)
    print("SNR is %f dB" % SNR)
    print("RMS: %f, SSIM: %f" % (rms_errors[-1], ssim_errors[-1]))
