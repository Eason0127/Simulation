...
# acquire hologram
hologram_field = angular_spectrum_method(field_after_object, 1.6e-6, z2, W, H, 1024)
...
# Update the grid
new_numPixels = undersampled_hologram_amplitude.shape[0]
x_us = np.arange(new_numPixels) - new_numPixels / 2 - 1
y_us = np.arange(new_numPixels) - new_numPixels / 2 - 1
W_new, H_new = np.meshgrid(x_us, y_us)

# --- Adding noise---

# 应该考虑第一泊松分布撞击像素，再考虑量子效率产生的电子，最后再用这个去带入噪声。
# 然后再去计算SNR，为信号的方差比上噪声的方差，信号的方差可能是光源的柏松分布，噪声就是各种噪声相加取根号。
...
# --- Run IPR ---
field_ite, rms_errors, ssim_errors = IPR(hologram_amplitude_noisy, z2, 80, 1.5e-20, 1.6e-6, W_new, H_new, 128, amp_field_after_ds)
...
plt.imshow(extended_rec_amp, cmap='gray')
