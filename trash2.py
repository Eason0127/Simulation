...
for pixelSize in sensor_pixel_sizes:
    ...
    # --- IPR 过程 ---
    def IPR(Measured_amplitude, distance, k_max, convergence_threshold, pixelSize, W, H, numPixels):
        update_phase = []
        last_field = None
        rms_errors = []
        ssim_errors = []
        ...
        for k in range(k_max):
            ...
            # 计算误差
            if k > 0:
                rms_error = np.sqrt(np.mean((amp_field3 - Measured_amplitude) ** 2))
    ...
    # --- 绘制图像 ---
    plt.figure(figsize=(6, 6))
    plt.imshow(np.abs(IPR_object), cmap='gray')
    plt.colorbar(label="Reconstructed Amplitude")
